...
class TaskManager(Module):
    ...
    def apply_updates(self, updates):
        """
        Locally applies the received updates of a task package generated by another
        manager.

        TODO: synchronization may require more than one message passing.
        """
        pkg_name = updates[0]
        pkg = self.task_packages.get(pkg_name, None)
        if pkg:
            ...
            # a new package; create its folder first
            pkg_folder = os.path.join(pydraSettings.tasks_dir, pkg_name)
            os.mkdir(pkg_folder)
            pkg = packaging.TaskPackage(pkg_folder)
            pkg.sync(updates[1])


    def push_sync(self, pkg_name):
        pass
